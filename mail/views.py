...
from .serializers import ProtonSerializer
...
class Folders(TemplateView):
    template_name = 'mail/folders.html'


class MailAPIView(APIView):
    def get(self, request):
        folders = mail.get_sorted_messages()
    ...
    def _messages(self, data):
        if data['status'] == 'read_message':
            read_message = mail.read_message(data['id'], True)
            context = {
                'url': 'messages',
                'status': 'read_message',
    ...
    def _read_message(self, data):
        if data['status'] == 'delete message':
            message_id = data['id']
            mail.delete_messages(message_id)
            return Response({'status': 'continue'})
